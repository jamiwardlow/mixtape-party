...
import { consumeAuthToken, issueAuthToken, PASSWORD_RESET_TTL_MS } from '../authTokens.js';
...
describe('auth tokens', () => {
  ...
  it('rejects a token presented for the wrong purpose and leaves it unused', async () => {
    const accountId = await createAccount();
    const token = await issueAuthToken(testDb.pool, accountId, 'sign_in', PASSWORD_RESET_TTL_MS);
